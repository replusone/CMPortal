...
if(empty($_POST) === false){
	...
	if(md5($_POST['current_password']) === $user_data['password'] && empty($errors) === true){
	...
	}else if(md5($_POST['current_password']) !== $user_data['password'] && empty($errors) === true){
...
if(isset($_GET['changed']) && empty($_GET['changed'])){
	...
	if((isset($_GET['force']) && empty($_GET['force'])) || $user_data['password_recover'] == 1){
		echo '<font color="red"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;You must change your password now in order to complete password recovery</font>';
	}
	if(empty($_POST) === false && empty($errors) === true){
		setcookie('password', $_POST['password'], time()+9999);
		change_password($session_user_id, $_POST['password']);
		header('Location: changepassword.php?changed');	
		//exit();
...
?>
	<form action="" method="post">
		<div class="row">
		    ...
		    <div class="input-field col s3">
		        <input class="btn-large waves-effect waves-light left" type="submit" name="change_password" value="Change Password">
			</div>
		</div>
	</form>
<?php
